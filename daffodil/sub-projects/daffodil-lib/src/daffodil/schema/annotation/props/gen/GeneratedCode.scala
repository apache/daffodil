...
trait TextStringJustificationMixin extends PropertyMixin {
  ...
  def textStringJustificationToString() = {
    optionTextStringJustification match {
      ...
      case Some(textStringJustification) =>  "dfdl:textStringJustification='" + textStringJustification + "'"
    }
  }
...
trait TextNumberJustificationMixin extends PropertyMixin {
  ...
  def textNumberJustificationToString() = {
    optionTextNumberJustification match {
      case None => "" // empty string if not present
      case Some(textNumberJustification) =>  "dfdl:textNumberJustification='" + textNumberJustification + "'"
    }
  }
...
trait TextNumberRoundingMixin extends PropertyMixin {
  ...
  def textNumberRoundingToString() = {
    optionTextNumberRounding match {
      case None => "" // empty string if not present
      case Some(textNumberRounding) =>  "dfdl:textNumberRounding='" + textNumberRounding + "'"
    }
  }
...
trait SignCodingValueMixin extends PropertyMixin {
  ...
  def signCodingValueToString() = {
    optionSignCodingValue match {
      case None => "" // empty string if not present
      case Some(signCodingValue) =>  "dfdl:signCodingValue='" + signCodingValue + "'"
    }
  }
...
trait RepresentationMixin extends PropertyMixin {
  ...
  def representationToString() = {
    optionRepresentation match {
      case None => "" // empty string if not present
      case Some(representation) =>  "dfdl:representation='" + representation + "'"
    }
  }
...
trait TextPadKindMixin extends PropertyMixin {
  ...
  def textPadKindToString() = {
    optionTextPadKind match {
      case None => "" // empty string if not present
      case Some(textPadKind) =>  "dfdl:textPadKind='" + textPadKind + "'"
    }
  }
...
trait TextTrimKindMixin extends PropertyMixin {
  ...
  def textTrimKindToString() = {
    optionTextTrimKind match {
      case None => "" // empty string if not present
      case Some(textTrimKind) =>  "dfdl:textTrimKind='" + textTrimKind + "'"
    }
  }
...
trait EscapeKindMixin extends PropertyMixin {
  ...
  def escapeKindToString() = {
    optionEscapeKind match {
      case None => "" // empty string if not present
      case Some(escapeKind) =>  "dfdl:escapeKind='" + escapeKind + "'"
    }
  }
...
trait GenerateEscapeMixin extends PropertyMixin {
  ...
  def generateEscapeToString() = {
    optionGenerateEscape match {
      case None => "" // empty string if not present
      case Some(generateEscape) =>  "dfdl:generateEscape='" + generateEscape + "'"
    }
  }
...
trait TextBidiTextOrderingMixin extends PropertyMixin {
  ...
  def textBidiTextOrderingToString() = {
    optionTextBidiTextOrdering match {
      case None => "" // empty string if not present
      case Some(textBidiTextOrdering) =>  "dfdl:textBidiTextOrdering='" + textBidiTextOrdering + "'"
    }
  }
...
trait TextBidiOrientationMixin extends PropertyMixin {
  ...
  def textBidiOrientationToString() = {
    optionTextBidiOrientation match {
      case None => "" // empty string if not present
      case Some(textBidiOrientation) =>  "dfdl:textBidiOrientation='" + textBidiOrientation + "'"
    }
  }
...
trait TextBidiNumeralShapesMixin extends PropertyMixin {
  ...
  def textBidiNumeralShapesToString() = {
    optionTextBidiNumeralShapes match {
      case None => "" // empty string if not present
      case Some(textBidiNumeralShapes) =>  "dfdl:textBidiNumeralShapes='" + textBidiNumeralShapes + "'"
    }
  }
...
trait TextNumberRepMixin extends PropertyMixin {
  ...
  def textNumberRepToString() = {
    optionTextNumberRep match {
      case None => "" // empty string if not present
      case Some(textNumberRep) =>  "dfdl:textNumberRep='" + textNumberRep + "'"
    }
  }
...
////////////////////////////////////////////////////////////////////////
// <xsd:simpleType name="NumberCheckPolicyEnum" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:dfdl="http://www.ogf.org/dfdl/dfdl-1.0/">
// 		<xsd:restriction base="xsd:string">
...
sealed trait NumberCheckPolicy extends NumberCheckPolicy.Value
object NumberCheckPolicy extends Enum[NumberCheckPolicy] {
  case object Strict extends NumberCheckPolicy ; forceConstruction(Strict)
  case object Lax extends NumberCheckPolicy ; forceConstruction(Lax)

  def apply(name: String) : NumberCheckPolicy = stringToEnum("numberCheckPolicy", name)
}
  
trait NumberCheckPolicyMixin extends PropertyMixin {
    
  /**
   * get property value, or fail trying. Use this if you need
   * the property value.
   */
  lazy val numberCheckPolicy = NumberCheckPolicy(getProperty("numberCheckPolicy"))
   ...
   * Mostly do not use this. Most code shouldn't need to test for 
   * property existence. Just insist on the property you need by
   * calling getProperty("numberCheckPolicy")
   */
  lazy val optionNumberCheckPolicy = getPropertyOption("numberCheckPolicy")
    
  /**
   * This will print the property value if the property has any value
   * in scope. This is mostly for debugging purposes.
   */   
  def numberCheckPolicyToString() = {
    optionNumberCheckPolicy match {
      case None => "" // empty string if not present
      case Some(numberCheckPolicy) =>  "dfdl:numberCheckPolicy='" + numberCheckPolicy + "'"
    }
  }
    
  def numberCheckPolicyInit() = {
    registerToStringFunction(numberCheckPolicyToString)
  }
    
  numberCheckPolicyInit() // call at object creation to initialize
}
...
trait CalendarCheckPolicyMixin extends PropertyMixin {
  ...
  def calendarCheckPolicyToString() = {
    optionCalendarCheckPolicy match {
      case None => "" // empty string if not present
      case Some(calendarCheckPolicy) =>  "dfdl:calendarCheckPolicy='" + calendarCheckPolicy + "'"
    }
  }
...
////////////////////////////////////////////////////////////////////////
// <xsd:simpleType name="NumberRoundingModeEnum" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:dfdl="http://www.ogf.org/dfdl/dfdl-1.0/">
// 		<xsd:restriction base="xsd:string">
...
sealed trait NumberRoundingMode extends NumberRoundingMode.Value
object NumberRoundingMode extends Enum[NumberRoundingMode] {
  case object RoundCeiling extends NumberRoundingMode ; forceConstruction(RoundCeiling)
  case object RoundFloor extends NumberRoundingMode ; forceConstruction(RoundFloor)
  case object RoundDown extends NumberRoundingMode ; forceConstruction(RoundDown)
  case object RoundUp extends NumberRoundingMode ; forceConstruction(RoundUp)
  case object RoundHalfEven extends NumberRoundingMode ; forceConstruction(RoundHalfEven)
  case object RoundHalfDown extends NumberRoundingMode ; forceConstruction(RoundHalfDown)
  case object RoundHalfUp extends NumberRoundingMode ; forceConstruction(RoundHalfUp)
  case object None extends NumberRoundingMode ; forceConstruction(None)

  def apply(name: String) : NumberRoundingMode = stringToEnum("numberRoundingMode", name)
}
  
trait NumberRoundingModeMixin extends PropertyMixin {
    
  /**
   * get property value, or fail trying. Use this if you need
   * the property value.
   */
  lazy val numberRoundingMode = NumberRoundingMode(getProperty("numberRoundingMode"))
   ...
   * Mostly do not use this. Most code shouldn't need to test for 
   * property existence. Just insist on the property you need by
   * calling getProperty("numberRoundingMode")
   */
  lazy val optionNumberRoundingMode = getPropertyOption("numberRoundingMode")
    
  /**
   * This will print the property value if the property has any value
   * in scope. This is mostly for debugging purposes.
   */   
  def numberRoundingModeToString() = {
    optionNumberRoundingMode match {
      case None => "" // empty string if not present
      case Some(numberRoundingMode) =>  "dfdl:numberRoundingMode='" + numberRoundingMode + "'"
    }
  }
    
  def numberRoundingModeInit() = {
    registerToStringFunction(numberRoundingModeToString)
  }
    
  numberRoundingModeInit() // call at object creation to initialize
}

////////////////////////////////////////////////////////////////////////
// <xsd:simpleType name="NumberZonedSignStyleEnum" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:dfdl="http://www.ogf.org/dfdl/dfdl-1.0/">
// 		<xsd:restriction base="xsd:string">
...
sealed trait NumberZonedSignStyle extends NumberZonedSignStyle.Value
object NumberZonedSignStyle extends Enum[NumberZonedSignStyle] {
  case object AsciiStandard extends NumberZonedSignStyle ; forceConstruction(AsciiStandard)
  case object AsciiTranslatedEBCDIC extends NumberZonedSignStyle ; forceConstruction(AsciiTranslatedEBCDIC)
  case object AsciiCARealiaModified extends NumberZonedSignStyle ; forceConstruction(AsciiCARealiaModified)

  def apply(name: String) : NumberZonedSignStyle = stringToEnum("numberZonedSignStyle", name)
}
  
trait NumberZonedSignStyleMixin extends PropertyMixin {
    
  /**
   * get property value, or fail trying. Use this if you need
   * the property value.
   */
  lazy val numberZonedSignStyle = NumberZonedSignStyle(getProperty("numberZonedSignStyle"))
   ...
   * Mostly do not use this. Most code shouldn't need to test for 
   * property existence. Just insist on the property you need by
   * calling getProperty("numberZonedSignStyle")
   */
  lazy val optionNumberZonedSignStyle = getPropertyOption("numberZonedSignStyle")
    
  /**
   * This will print the property value if the property has any value
   * in scope. This is mostly for debugging purposes.
   */   
  def numberZonedSignStyleToString() = {
    optionNumberZonedSignStyle match {
      case None => "" // empty string if not present
      case Some(numberZonedSignStyle) =>  "dfdl:numberZonedSignStyle='" + numberZonedSignStyle + "'"
    }
  }
    
  def numberZonedSignStyleInit() = {
    registerToStringFunction(numberZonedSignStyleToString)
  }
    
  numberZonedSignStyleInit() // call at object creation to initialize
}
...
trait BinaryNumberRepMixin extends PropertyMixin {
  ...
  def binaryNumberRepToString() = {
    optionBinaryNumberRep match {
      case None => "" // empty string if not present
      case Some(binaryNumberRep) =>  "dfdl:binaryNumberRep='" + binaryNumberRep + "'"
    }
  }
...
trait BinaryNumberCheckPolicyMixin extends PropertyMixin {
  ...
  def binaryNumberCheckPolicyToString() = {
    optionBinaryNumberCheckPolicy match {
      case None => "" // empty string if not present
      case Some(binaryNumberCheckPolicy) =>  "dfdl:binaryNumberCheckPolicy='" + binaryNumberCheckPolicy + "'"
    }
  }
...
trait BinaryFloatRepMixin extends PropertyMixin {
  ...
  def binaryFloatRepToString() = {
    optionBinaryFloatRep match {
      case None => "" // empty string if not present
      case Some(binaryFloatRep) =>  "dfdl:binaryFloatRep='" + binaryFloatRep + "'"
    }
  }
...
trait TextBooleanJustificationMixin extends PropertyMixin {
  ...
  def textBooleanJustificationToString() = {
    optionTextBooleanJustification match {
      case None => "" // empty string if not present
      case Some(textBooleanJustification) =>  "dfdl:textBooleanJustification='" + textBooleanJustification + "'"
    }
  }
...
trait TextCalendarJustificationMixin extends PropertyMixin {
  ...
  def textCalendarJustificationToString() = {
    optionTextCalendarJustification match {
      case None => "" // empty string if not present
      case Some(textCalendarJustification) =>  "dfdl:textCalendarJustification='" + textCalendarJustification + "'"
    }
  }
...
trait CalendarPatternKindMixin extends PropertyMixin {
  ...
  def calendarPatternKindToString() = {
    optionCalendarPatternKind match {
      case None => "" // empty string if not present
      case Some(calendarPatternKind) =>  "dfdl:calendarPatternKind='" + calendarPatternKind + "'"
    }
  }
...
trait CalendarFirstDayOfWeekMixin extends PropertyMixin {
  ...
  def calendarFirstDayOfWeekToString() = {
    optionCalendarFirstDayOfWeek match {
      case None => "" // empty string if not present
      case Some(calendarFirstDayOfWeek) =>  "dfdl:calendarFirstDayOfWeek='" + calendarFirstDayOfWeek + "'"
    }
  }
...
trait BinaryCalendarRepMixin extends PropertyMixin {
  ...
  def binaryCalendarRepToString() = {
    optionBinaryCalendarRep match {
      case None => "" // empty string if not present
      case Some(binaryCalendarRep) =>  "dfdl:binaryCalendarRep='" + binaryCalendarRep + "'"
    }
  }
...
trait LengthKindMixin extends PropertyMixin {
  ...
  def lengthKindToString() = {
    optionLengthKind match {
      case None => "" // empty string if not present
      case Some(lengthKind) =>  "dfdl:lengthKind='" + lengthKind + "'"
    }
  }
...
trait NilKindMixin extends PropertyMixin {
  ...
  def nilKindToString() = {
    optionNilKind match {
      case None => "" // empty string if not present
      case Some(nilKind) =>  "dfdl:nilKind='" + nilKind + "'"
    }
  }
...
trait NilValueDelimiterPolicyMixin extends PropertyMixin {
  ...
  def nilValueDelimiterPolicyToString() = {
    optionNilValueDelimiterPolicy match {
      case None => "" // empty string if not present
      case Some(nilValueDelimiterPolicy) =>  "dfdl:nilValueDelimiterPolicy='" + nilValueDelimiterPolicy + "'"
    }
  }
...
trait EmptyValueDelimiterPolicyMixin extends PropertyMixin {
  ...
  def emptyValueDelimiterPolicyToString() = {
    optionEmptyValueDelimiterPolicy match {
      case None => "" // empty string if not present
      case Some(emptyValueDelimiterPolicy) =>  "dfdl:emptyValueDelimiterPolicy='" + emptyValueDelimiterPolicy + "'"
    }
  }
...
trait SequenceKindMixin extends PropertyMixin {
  ...
  def sequenceKindToString() = {
    optionSequenceKind match {
      case None => "" // empty string if not present
      case Some(sequenceKind) =>  "dfdl:sequenceKind='" + sequenceKind + "'"
    }
  }
...
trait LengthUnitsMixin extends PropertyMixin {
  ...
  def lengthUnitsToString() = {
    optionLengthUnits match {
      case None => "" // empty string if not present
      case Some(lengthUnits) =>  "dfdl:lengthUnits='" + lengthUnits + "'"
    }
  }
...
trait AlignmentUnitsMixin extends PropertyMixin {
  ...
  def alignmentUnitsToString() = {
    optionAlignmentUnits match {
      case None => "" // empty string if not present
      case Some(alignmentUnits) =>  "dfdl:alignmentUnits='" + alignmentUnits + "'"
    }
  }
...
trait GenerateQuotesMixin extends PropertyMixin {
  ...
  def generateQuotesToString() = {
    optionGenerateQuotes match {
      case None => "" // empty string if not present
      case Some(generateQuotes) =>  "dfdl:generateQuotes='" + generateQuotes + "'"
    }
  }
...
trait SeparatorPositionMixin extends PropertyMixin {
  ...
  def separatorPositionToString() = {
    optionSeparatorPosition match {
      case None => "" // empty string if not present
      case Some(separatorPosition) =>  "dfdl:separatorPosition='" + separatorPosition + "'"
    }
  }
...
trait SeparatorPolicyMixin extends PropertyMixin {
  ...
  def separatorPolicyToString() = {
    optionSeparatorPolicy match {
      case None => "" // empty string if not present
      case Some(separatorPolicy) =>  "dfdl:separatorPolicy='" + separatorPolicy + "'"
    }
  }
...
trait ChoiceLengthKindMixin extends PropertyMixin {
  ...
  def choiceLengthKindToString() = {
    optionChoiceLengthKind match {
      case None => "" // empty string if not present
      case Some(choiceLengthKind) =>  "dfdl:choiceLengthKind='" + choiceLengthKind + "'"
    }
  }
...
trait OccursCountKindMixin extends PropertyMixin {
  ...
  def occursCountKindToString() = {
    optionOccursCountKind match {
      case None => "" // empty string if not present
      case Some(occursCountKind) =>  "dfdl:occursCountKind='" + occursCountKind + "'"
    }
  }
...
trait ByteOrderMixin extends PropertyMixin {
  ...
  def byteOrderToString() = {
    optionByteOrder match {
      case None => "" // empty string if not present
      case Some(byteOrder) =>  "dfdl:byteOrder='" + byteOrder + "'"
    }
  }
...
trait YesNoMixin extends PropertyMixin {
  ...
  def yesNoToString() = {
    optionYesNo match {
      case None => "" // empty string if not present
      case Some(yesNo) =>  "dfdl:yesNo='" + yesNo + "'"
    }
  }
...
trait UTF16WidthMixin extends PropertyMixin {
  ...
  def utf16WidthToString() = {
    optionUTF16Width match {
      case None => "" // empty string if not present
      case Some(utf16Width) =>  "dfdl:utf16Width='" + utf16Width + "'"
    }
  }
...
trait TestKindMixin extends PropertyMixin {
  ...
  def testKindToString() = {
    optionTestKind match {
      case None => "" // empty string if not present
      case Some(testKind) =>  "dfdl:testKind='" + testKind + "'"
    }
  }
...
// 		<xsd:attribute type="dfdl:NumberCheckPolicyEnum" name="textNumberCheckPolicy"></xsd:attribute>
...
// 		<xsd:attribute type="dfdl:NumberRoundingModeEnum" name="textNumberRoundingMode"></xsd:attribute>
...
// 		<xsd:attribute type="dfdl:NumberZonedSignStyleEnum" name="textZonedSignStyle"></xsd:attribute>
...
trait TextNumberFormatAGMixin extends PropertyMixin
  ...
  lazy val textNumberCheckPolicy = NumberCheckPolicy(getProperty("textNumberCheckPolicy"))
  ...
  lazy val textNumberRoundingMode = NumberRoundingMode(getProperty("textNumberRoundingMode"))
  lazy val textNumberRoundingIncrement = convertToDouble(getProperty("textNumberRoundingIncrement"))
  lazy val textZonedSignStyle = NumberZonedSignStyle(getProperty("textZonedSignStyle"))
...
trait PropertyNameTypeMixin extends PropertyMixin {
  ...
  def propertyNameTypeToString() = {
    optionPropertyNameType match {
      case None => "" // empty string if not present
      case Some(propertyNameType) =>  "dfdl:propertyNameType='" + propertyNameType + "'"
    }
  }
