...
public class PythonGenerator extends Generator {
    ...
    public PythonGenerator(WriterFactory factory) {
      super(factory);
    }

    @Override public Node prepare(Node root) {
        return prepareInternal(root, new PythonImportExtractor());
    }
...
//        }
    ...
    @Override public void generateFile(List<Import> imports, ClassDeclaration mainClass, List<ClassDeclaration> innerClasses) {
        ...
        for (Import imp : imports) {
            writer.writeLine("import " + ((PythonImportExtractor.PythonImport) imp).getIdentifier());
        }
        writer.writeLine("");
        ...
        // Add a file output
        files.add(new OutputFile(mainClass.getClassName() + ".java", content));
    }
