...
public class JavaGenerator extends Generator {
    ...
    public JavaGenerator(WriterFactory factory) {
        super(factory);
    }

    @Override public Node prepare(Node root) {
        return prepareInternal(root, new JavaImportExtractor());
    }
    ...
    @Override public void generateFile(List<Import> imports, ClassDeclaration mainClass, List<ClassDeclaration> innerClasses) {
        ...
        // Add Content
        writer.writeLine("namespace " + mainClass.getNamespace() + ";");
        writer.writeLine("");
        for (Import imp : imports) {
            writer.writeLine("import " + ((JavaImportExtractor.JavaPackageImport) imp).getTypeString() + ";");
        }
        writer.writeLine("");
        ...
        // Add a file output
        files.add(new OutputFile(mainClass.getNamespace().replace(".", "/") + "/" + mainClass.getClassName() + ".java", content));
    }
