...
class TwoByteSwapLayer extends ByteSwap("twobyteswap", 2)
class FourByteSwapLayer extends ByteSwap("fourbyteswap", 4)

abstract class ByteSwap(name: String, count: Int)
  extends Layer(
    name,
    supportedLayerLengthKinds =
      Seq(JLayerLengthKind.Explicit, JLayerLengthKind.Implicit).asJava,
