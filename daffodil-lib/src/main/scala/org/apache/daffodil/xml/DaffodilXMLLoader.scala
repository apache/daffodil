...
class DaffodilXMLLoader(val errorHandler: org.xml.sax.ErrorHandler)
  ...
  def validateAsXMLSchema(source: DaffodilSchemaSource): Unit = {
    ...
    load(source, Some(XMLUtils.schemaForDFDLSchemas))
    //
   ...
   */
  def load(source: DaffodilSchemaSource, optSchemaURI: Option[URI]): scala.xml.Node = {
    //
  ...
  override def load(url: URL): Node = noWay
  ...
  override def loadFile(f: File) = noWay
  override def loadFile(fd: java.io.FileDescriptor): Node = noWay
  override def loadFile(name: String): Node = noWay
  // $CONVERAGE-ON$
