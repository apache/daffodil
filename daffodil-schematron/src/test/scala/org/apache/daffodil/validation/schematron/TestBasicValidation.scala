...
class TestBasicValidation {
  ...
  @Test def testValidXML(): Unit = {
    ...
    val p = SchematronValidatorFactory.makeValidator(new ByteArrayInputStream(sch.getBytes))
  ...
  @Test def testInvalidXML2(): Unit = {
    ...
    val p = SchematronValidatorFactory.makeValidator(new ByteArrayInputStream(sch.getBytes), Some(new CustomResolver))
  ...
  @Test def testInstantiateAnInstanceOfTemplates(): Unit = {
    ...
    val rules = Schematron.templatesFor(getClass.getClassLoader.getResourceAsStream("sch/schematron-1.sch"), factory)
    assertNotNull(rules)
  }
