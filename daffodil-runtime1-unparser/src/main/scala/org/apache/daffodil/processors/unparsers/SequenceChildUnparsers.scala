...
abstract class RepeatingChildUnparser(
   ...
   * the answer is false.
   */
  def shouldDoUnparser(unparser: RepeatingChildUnparser, state: UState): Boolean = {
    ...
    val chk: Boolean =
      if (res) {
        ...
        if (!check) {
          // System.err.println("Stopping occurrences(5) of %s due to array pos %s".format(erd.namedQName, state.arrayPos))
        }
        check
      } else
        false
   ...
   */
  def checkOccursCount(state: UState,
