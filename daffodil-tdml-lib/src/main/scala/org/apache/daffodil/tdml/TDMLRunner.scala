...
case class ParserTestCase(ptc: NodeSeq, parentArg: DFDLTestSuite)
    ...
    val (parseResult, diagnostics, isError) = {
      ...
      val isErr: Boolean =
        if (actual.isProcessingError) true
          ...
          //
          val loc: api.DataLocation = actual.currentLocation
      ...
      val diagnostics = compileWarnings ++ actual.getDiagnostics.asScala
  ...
  private def doParseExpectSuccess(
    ...
    val diagObjs = actual.getDiagnostics.asScala
  ...
  private def verifyParseResults(
    ...
    val allDiags = compileWarnings ++ actual.getDiagnostics.asScala
    checkDiagnosticMessages(
      allDiags,
      None,
      optExpectedWarnings,
...
case class UnparserTestCase(ptc: NodeSeq, parentArg: DFDLTestSuite)
  ...
  def runUnparserExpectSuccess(
    ...
    val allDiags = actual.getDiagnostics.asScala ++ compileWarnings
    checkDiagnosticMessages(
      allDiags,
      None,
      optWarnings,
  ...
  def runUnparserExpectErrors(
    ...
    val diagnostics = compileWarnings ++ actual.getDiagnostics.asScala ++ dataErrors
...
object VerifyTestCase {
  ...
  def verifyDiagnosticsFound(
    actualDiagsFiltered: Iterable[api.Diagnostic],
 ...
 * tests. We use a ListMap here because usually there will only be single entry,
  ...
  def getCompileResult(
    ...
    val compileResultCacheValue =
      if (optVal.isDefined) {
        ...
        // the compile result does not have validation enabled, store the result in the
    ...
    // we defintely have a compileResultMap that contains a CompileResult for "off"
    // validation. If we don't have a CompileResult for the requested validation type,
    // then get the "off" CompileResult and create a new CompileResult with a processor
    // that does have the requested validation, and add it to the compileResultMap.
