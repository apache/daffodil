 ...
 *
 * We inline all methods because attempts to call non-inlined methods trigger boxing
 */
...
object DataValue {
   ...
   * so we instead punt on this issue and require the caller to translate to/from AnyRef at the boundery.
   *
