...
trait NextElementResolver {
  ...
  final def nextElement(name: String, nameSpace: String, hasNamespace: Boolean): ElementRuntimeData = {
    ...
    while (iter.hasNext && maybeERD.isEmpty) {
      val trd = iter.next
      trd match {
        case erd: ElementRuntimeData if (erd.isComplexType) => {
          //
          // We made it down to a complex type.
          // That means we can't be expecting another child element
          // as we're expecting an end-of element event instead
          //
          error(iter, name, nameSpace, Some(trd))
        }
    ...
    //
    if (maybeERD.isEmpty) {
      error(iter, name, nameSpace, None)
    }
    maybeERD.get
  }
  ...
  private def error(
    ...
    name: String,
    namespace: String,
    optTRD: Option[TermRuntimeData]): Unit = {
    val sqn = StepQName(None, name, NS(namespace))
    val allTRDs = {
      iter.reset()
    ...
    val allPossibles =
      allTRDs.flatMap {
        _.partialNextElementResolver.allPossibleNextElements.map {
          _.namedQName.toExtendedSyntax
        }
      }
    val allPossiblesString = allPossibles.mkString(", ")
    val maybeLoc: Maybe[SchemaFileLocation] = Maybe.toMaybe(optTRD.map { _.schemaFileLocation })
    allPossibles.length match {
      case 0 =>
        UnparseError(maybeLoc, Nope,
          "Found element %s, but no start of element is expected.",
          sqn.toExtendedSyntax)
      case 1 =>
        UnparseError(maybeLoc, Nope,
          "Found element %s, but expected %s.",
          sqn.toExtendedSyntax, allPossiblesString)
      case _ =>
        UnparseError(maybeLoc, Nope,
          "Found element %s, but expected one of %s.",
          sqn.toExtendedSyntax, allPossiblesString)
    }
  }
}
...
class NoNextElement(schemaFileLocation: SchemaFileLocation) extends PartialNextElementResolver {
  ...
  override def maybeNextElement(local: String, namespace: String, hasNamespace: Boolean): Maybe[ElementRuntimeData] = {
    ...
    UnparseError(One(schemaFileLocation), Nope, "Found element %s, but no element is expected.", sqn)
    Maybe.Nope
  }
  ...
  override def maybeNextElement(local: String, namespace: String, hasNamespace: Boolean): Maybe[ElementRuntimeData] = {
    ...
    if (!matches) {
      if (isRequiredInInfoset) {
        val sqn = StepQName(None, local, NS(namespace))
        UnparseError(One(schemaFileLocation), Nope, "Found element %s, but expected %s.",
...
class SeveralPossibilitiesForNextElement(
  ...
  override def maybeNextElement(local: String, namespace: String, hasNamespace: Boolean): Maybe[ElementRuntimeData] = {
      ...
      if (hasNamespace) {
        ...
        if (!hasDuplicateLocalNames) {
          ...
          if (localMatches.size > 1) {
            ...
            UnparseError(One(loc), Nope, "Found multiple matches for element %s because infoset implementation ignores namespaces. Matches are %s",
              sqn.toExtendedSyntax, keys.mkString(", "))
          }
    ...
    val res = optnextERD
    if (isRequiredInInfoset && res.isEmpty) {
      val keys = nextERDMap.keys.toSeq
      val sqn = StepQName(None, local, NS(namespace))
      UnparseError(One(loc), Nope, "Found element %s, but expected one of %s.",
        sqn.toExtendedSyntax, keys.mkString(", "))
    }
    res
  }
