...
//
//private[unparsers] object XMLInfosetEventIterator {
//  sealed trait Mode
...
// */
//class XMLInfosetEventIterator(xsr: XMLEventCursor) extends Iterator[XMLInfosetEvent] {
//
//  import XMLInfosetEventIterator._
//
//  // FIXME: Performance: This is a complete hack currently. This should be revised to use Cursor/Accessor
//  // so that all this allocating and copying of objects doesn't have to happen.
//  private lazy val iterator = trans(preprocess(xsr.toStream.map { _.event }), new mutable.ArrayStack[scala.xml.NamespaceBinding]()).toIterator
...
//    xes match {
//      case EvStart(_, _, attrs, scope) #:: EvStart(_, _, _, _) #:: rest => ElementOnly
//      case EvStart(_, _, attrs, scope) #:: EvText(s) #:: EvStart(_, _, _, _) #:: rest => {
//        if (!s.matches("""\s*""")) Assert.invariantFailed("couldn't determine Mode for " + xes)
//        ElementOnly
//      }
//      case EvStart(_, _, attrs, scope) #:: EvEnd(_, _) #:: rest if isNil(attrs, scope) => NilContent
//      case EvStart(_, _, attrs, scope) #:: EvEnd(_, _) #:: rest => SimpleContent
//      case EvStart(_, _, _, _) #:: EvText(s) #:: EvEnd(_, _) #:: rest => SimpleContent
//      case EvStart(_, _, _, _) #:: EvText(s) #:: EvStart(_, _, _, _) #:: rest if s.matches("""\s*""") => ElementOnly
//      case EvStart(_, _, _, _) #:: EvText(s) #:: rest if !s.matches("""\s*""") => SimpleContent
//      case EvStart(_, _, _, _) #:: EvEntityRef(s) #:: rest => SimpleContent
...
//    val hd #:: tl = xes
//    hd match {
//      case start @ EvStart(pre, label, attrs, scope) => {
...
//    tl: Stream[XMLEvent],
//    scopeStack: mutable.ArrayStack[scala.xml.NamespaceBinding]): Stream[XMLInfosetEvent] = {
//    val simpleParts = tl.takeWhile { ev => !ev.isInstanceOf[EvEnd] }.toList
//    val EvEnd(pre, `label`) #:: after = tl.drop(simpleParts.length)
//    Assert.invariant(ns =:= scopeStack.top.getURI(pre))
