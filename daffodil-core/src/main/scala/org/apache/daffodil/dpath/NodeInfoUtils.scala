...
object NodeInfoUtils {
  ...
  def generalizeArgAndResultTypesForNumericOp(op: String,
    ...
    val (argType: Numeric.Kind, resultType: Numeric.Kind) = (leftArgType, rightArgType) match {
      ...
      case (lat, NonNegativeInteger) if (lat.isInstanceOf[SignedInteger.Kind]) => (Integer, Integer)
      case (_, NonNegativeInteger) => (NonNegativeInteger, Integer)
      case (NonNegativeInteger, rat) if (rat.isInstanceOf[SignedInteger.Kind]) => (Integer, Integer)
      case (NonNegativeInteger, _) => (NonNegativeInteger, Integer)
      case (lat, UnsignedLong) if (lat.isInstanceOf[SignedInteger.Kind]) => (Integer, Integer)
      case (_, UnsignedLong) => (UnsignedLong, Integer)
      case (UnsignedLong, rat) if (rat.isInstanceOf[SignedInteger.Kind]) => (Integer, Integer)
      ...
      case (lat, UnsignedInt) if (lat.isInstanceOf[SignedInteger.Kind]) => (Long, Long)
      case (_, UnsignedInt) => (UnsignedInt, Long)
      case (UnsignedInt, rat) if (rat.isInstanceOf[SignedInteger.Kind]) => (Long, Long)
      case (UnsignedInt, _) => (UnsignedInt, Long)
      case (_, Int) => (Int, Int)
      case (Int, _) => (Int, Int)
      case (lat, UnsignedShort) if (lat.isInstanceOf[SignedInteger.Kind]) => (Int, Int)
      case (_, UnsignedShort) => (UnsignedShort, Int)
      case (UnsignedShort, rat) if (rat.isInstanceOf[SignedInteger.Kind]) => (Int, Int)
      case (UnsignedShort, _) => (UnsignedShort, Int)
      case (_, Short) => (Short, Int)
      case (Short, _) => (Short, Int)
      case (lat, UnsignedByte) if (lat.isInstanceOf[SignedInteger.Kind]) => (Short, Short)
      case (_, UnsignedByte) => (UnsignedByte, Short)
      case (UnsignedByte, rat) if (rat.isInstanceOf[SignedInteger.Kind]) => (Short, Short)
