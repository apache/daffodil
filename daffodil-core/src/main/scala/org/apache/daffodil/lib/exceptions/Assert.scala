...
abstract class ThinException protected (dummy: Int, cause: Throwable, fmt: String, args: Any*)
  ...
  def this(fmt: String, args: Any*) =
    this(1, null, fmt, args: _*) // Fix varargs expansion
 ...
 * @tparam T
 */
class MultiException[T <: Throwable] private (causes: java.util.List[T], msg: String, cause: T)
  extends ThinException(msg, cause) {
  def this(causes: java.util.List[T]) = this(
    causes,
    null,
    causes.asScala.headOption.getOrElse(
      Assert.usageError("There must be one or more causes.")
    )
  )
  private lazy val msgs = causes.asScala.map { Misc.getAMessage(_) }.mkString("\n")
  override def getMessage(): String = msgs
  def getCauses(): java.util.List[T] = causes
  def toss: Nothing =
    if (causes.size == 1)
      throw cause
    else
