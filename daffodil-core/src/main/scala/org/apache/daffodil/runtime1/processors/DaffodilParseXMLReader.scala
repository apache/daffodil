 ...
 */
class DaffodilParseXMLReader(dp: DataProcessor) extends api.DaffodilParseXMLReader {
  ...
  def parse(isdis: api.InputSourceDataInputStream): Unit = {
    ...
    if (diagnostics.size() > 0 && eh != null) {
      diagnostics.forEach { d =>
        val spe = {
          ...
          val (lineNo, colNo, systemId) = d.getLocationsInSchemaFiles.asScala.headOption
            .map { s =>
