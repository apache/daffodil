 ...
 */
final class InfosetInputter(delegate: api.infoset.InfosetInputter)
  ...
  def initialize(rootElementInfo: ElementRuntimeData, tunableArg: DaffodilTunables): Unit = {
    ...
    try {
      if (
        !delegate.hasNext || delegate.getEventType != InfosetInputterEventType.StartDocument
      ) {
        UnparseError(
  ...
  private def reallyFill(advanceInput: Boolean): Boolean = {
    ...
    if (!delegate.hasNext) {
      Assert.invariant(delegate.getEventType == EndDocument)
      fini()
      false
    } else
      true
  ...
  private def nextElementErd() =
    nextElement(
      delegate.getLocalName,
      delegate.getNamespaceURI,
      delegate.getSupportsNamespaces
    )
  ...
  private def createElement(erd: ERD) = {
    ...
    if (erd.isSimpleType) {
      val txt =
        try {
          delegate.getSimpleText(erd.optPrimType.get, erd.runtimeProperties)
        } catch {
  ...
  override def fini(): Unit = delegate.fini();
}
